...
void Input(void) {
    ...
    cout << "Number of bins for g(r) histogram = " << nbins << endl
         << endl;
    ReadInput.close();
    ...
    // Setup g(r) histogram variables
    max_r = box / 2.0;
    bin_size = max_r / nbins;

    gr = vector<double>(nbins, 0.0);
    gr_blk_av = vector<double>(nbins, 0.0);
    gr_glob_av = vector<double>(nbins, 0.0);
    gr_glob_av2 = vector<double>(nbins, 0.0);
    err_gr = vector<double>(nbins, 0.0); 
...
void Measure() { // Properties measurement
    ...
    for (int i = 0; i < npart - 1; ++i) {
        ...
        for (int j = i + 1; j < npart; ++j) {
            ...
            if(dr < max_r) {
                ...
                uint32_t idx = uint32_t(floor(dr / bin_size));
                gr[idx] += 2.0;
    ...
    vtail = 8.0 * M_PI * rho / 3.0 * (1 / (3.0 * pow(rcut, 9)) - 1 / pow(rcut, 3)) * npart;     // Potential energy correction in reduced units
    ptail = 32.0 * M_PI * rho * (1 / (3.0 * pow(rcut, 9)) - 1 / (2.0 * pow(rcut, 3))) * npart;  // Pressure correction in reduced units

    walker[iv] = 4.0 * v + vtail;                                                   // Potential energy with tail correction
    walker[ik] = kin;                                                               // Kinetic energy
    walker[it] = (2.0 / 3.0) * kin / (double)npart;                                 // Temperature
    walker[ie] = 4.0 * v + kin;                                                     // Total energy
    walker[iw] = rho * walker[it] + 1.0 / (3.0 * vol) * (48.0 * psum + ptail);      // Pressure with tail correction (tail correction acts on w)
...
void Averages(int iblk) { // Print results for current block
    ...
    for(size_t i = 0; i < nbins; ++i) {
        ...
        double r = i * bin_size;
        double shell_v = 4.0 / 3.0 * M_PI * (pow(bin_size + r, 3.0) - pow(r, 3.0));
        double gr_norm = rho * npart * shell_v;
        
        stima_gri = gr_blk_av[i] / blk_norm / gr_norm;
        gr_glob_av[i] += stima_gri;
        gr_glob_av2[i] += stima_gri * stima_gri;
        err_gr[i] = Error(gr_glob_av[i], gr_glob_av2[i], iblk);
    ...
    // Only last block average and error is needed for g(r) => write only on last block

    if (iblk == nblk) {

        for (size_t i = 0; i < nbins; ++i) {
            
            Gr << setw(wd) << i * bin_size;
        ...
        for (size_t i = 0; i < nbins; ++i) {
            ...
            Gr << setw(wd) << gr_glob_av[i] / (double)iblk;
