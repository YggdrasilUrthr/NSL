...
inline bool Random::In_Interval(double lower, double upper, double num) {
...
void Random::Init_Random_Gen(uint32_t PrimesLine, std::string SeedFile)
{
...
uint32_t Random::GenInt(uint32_t min, uint32_t max) {

    double r = Rannyu();

    double n_interv = max - min + 1.0;
    double step = 1.0 / n_interv;
