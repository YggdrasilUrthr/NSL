...
class DNN:
    ...
    def create_DNN(self):
        ...
        model.add(Dense(400,input_shape=(self.img_rows * self.img_cols,), activation='relu'))           # add a dense all-to-all relu layer
    ...
    def compile_model(self):
        self.create_DNN()                                                                   # create the model
    ...
    def train_DNN(self):
        ...
        self.history = self.model.fit(self.X_train, self.Y_train,                           # train DNN and store training info in history
                batch_size=self.batch_size,
                epochs=self.epochs,
                verbose=self.verbose,
                validation_data=(self.X_test, self.Y_test))
        
        return self.evaluate_DNN()

    def DNN_predict(self):
        self.predictions = self.model.predict(self.X_test)
        return self.predictions
    
    def save_model(self, filename_model = './DNN.keras', filename_history = './hist.npy'):
        self.model.save(filename_model)
        np.save(filename_history, self.history)

    def load_model(self, filename_model = './DNN.keras', filename_history = './hist.npy'):
